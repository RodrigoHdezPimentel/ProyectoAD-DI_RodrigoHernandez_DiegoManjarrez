...
import dam.prueba.spring_boot_foroex.models.GrupoUsuarioFK;
...
// JPQl no tiene soporte para usar el UNION >:(
    @Query(value = "SELECT  new dam.prueba.spring_boot_foroex.Class.ChatListUser(g,c) FROM Conversacion c JOIN GrupoUsuario g ON c.idgrupousuario = g.idgrupousuario" +
   ...
   List<ChatListUser> getListChatFromUser (Integer id);
    //UNION
   @Query(value = "SELECT new dam.prueba.spring_boot_foroex.Class.ChatListUser(g,c) FROM GrupoUsuario g LEFT JOIN Conversacion c" +

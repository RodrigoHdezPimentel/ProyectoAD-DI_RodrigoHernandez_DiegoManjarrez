...
public interface ConversacionRepository extends JpaRepository<Conversacion, Integer> {
    ...
    @Query(value = "SELECT new dam.prueba.spring_boot_foroex.Class.Message(c,u.idusuario,u.name) FROM Conversacion c JOIN GrupoUsuario gu " +
    ...
    @Query(value = "SELECT new dam.prueba.spring_boot_foroex.Class.Message(c,u.idusuario,u.name) FROM Conversacion c JOIN GrupoUsuario g " +
